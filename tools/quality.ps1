...
function Get-GdstyleAssetMetadata {
	if ($IsWindows -or $env:OS -eq "Windows_NT") {
		return @{
			...
			Sha256 = "66435774c2d41cbf58381216c52633a14e8888af9bb590d26730d20622f79bd6"
		}
	}
	if ($IsMacOS) {
		$machine = ""
		...
		if ($machine -eq "arm64" -or $machine -eq "aarch64") {
			return @{
				...
				Sha256 = "c2f417fc1a0c69d1be8239d5c7b37a85479f65f3a6b5e83bdeaf7d9125a21177"
			}
		}
		return @{
			Name = "gdstyle-x86_64-apple-darwin.tar.gz"
			Sha256 = "b8c359f57ded68a2c5b1446b6e11cdf03b67db446284f7a5727c59123c25ce48"
		}
	}
	return @{
		Name = "gdstyle-x86_64-unknown-linux-gnu.tar.gz"
		Sha256 = "96357d4040c21f3a61c13a7ed33fd6d5b505de0e455dd5b6caabf14706a08309"
	}
}
...
function Assert-GdstyleVersion {
	...
	$expectedNumber = [regex]::Escape($gdstyleVersion.TrimStart("v"))
	if ($versionOutput -notmatch $expectedNumber) {
