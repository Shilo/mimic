...
Code style: Follow the Godot GDScript style guide at https://docs.godotengine.org/en/4.4/tutorials/scripting/gdscript/gdscript_styleguide.html; since GDScript is close to Python, the guide is inspired by Python's PEP 8 programming style guide. Use tabs for indentation, UTF-8 text, snake_case for files/functions/variables/signals, PascalCase for class_name values and enum names, UPPER_CASE for constants, and \_private_name for private helpers or backing fields. Keep `class_name X extends Y` on one line when both are present; this project treats that as the preferred class declaration shape even though the upstream style guide usually separates them. Follow the style guide's code order explicitly, except for the one-line `class_name X extends Y` preference: annotations, class declaration/doc comment, signals, enums, constants, static variables, exports, regular variables, onready variables, static methods, built-in callbacks, public methods, private methods, then subclasses. The snake_case file naming rule applies to every repository file you create or rename, including Markdown, research, docs, scripts, scenes, and resources; use underscores, not hyphens, for multiword file names. Prefer explicit typed public API, guard clauses, small functions, minimal comments, and no unrelated formatting churn. Use current Godot 4, GDScript 2 patterns; do not use outdated Godot 3 or GDScript 1 habits. Prefer typed Callable usage such as `some_method.call_deferred(args)` over string-based `call_deferred("some_method", args)` for local methods. Prefer `class_name` scripts over `preload()` for addon script dependencies unless there is a concrete reason to avoid a global class. Document every public class, signal, enum, enum value, exported property, public variable, and public method with GDScript `##` documentation comments; do not document private API with `##`. Use `## [br][br]` for public documentation paragraph breaks so comments stay readable in code and render correctly in Godot tooltips. Document what each public method parameter and return value means as part of the method's `##` prose: GDScript has no `@param`/`@return` tags, and Godot already shows every parameter and return type from the typed signature in tooltips and the generated API site, so explain purpose and meaning and never restate the type. Reference each parameter by name with `[param name]` and cover its role, accepted values, units, and any sentinel or default behavior; separate each parameter and the return note with `## [br][br]` so every entry renders as its own paragraph in both editor tooltips and the generated API site (a single `## [br]` collapses them into one run-on paragraph on the site, which has `nl2br` disabled). Document the return when it carries meaning beyond its type, such as error semantics, sentinel values like `0`, empty, or `null`, or the `[enum ...]` a raw `int` represents; a getter whose summary already states exactly what it returns needs no extra return sentence. Reference the global error enum as `[enum Error]` and success as `[constant OK]`.

Typing guardrails: Use enum types for behavior-facing parameters, return values, signals, variables, and tests when a value is conceptually an enum. Use `int` only at raw storage/native boundaries such as `ProjectSettings`, untyped `Variant` data, peer IDs, ports, and external result codes that have no usable enum type; convert once at the behavior boundary and validate user-editable settings. Prefer explicit declarations like `var mode: Mimic.EditorAutoConnectMode = value`; do not combine inferred declarations with casts. Use `as` only for genuine Variant/object downcasts where direct typed assignment is not enough. Prefer small typed result classes over untyped dictionaries for structured multi-field helper returns that cross file boundaries.
